...
# EMalina, 2021-Dec-11, adding list input to save and load inventory methods
#------------------------------------------#
...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        """
        # split the file list into 2 items
                ...
                for disc in lst_Inventory:
                    ...
                    for row in tracks:
                        if row is None:
                            ...
                            tmp_track = '{},{}'.format(tmpcd_id, row.get_record())
                            trackfile.write(tmp_track)
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            with open(file_name_Track, 'r') as trackfile:
                for tRow in trackfile:
                    trackdata = tRow.strip().split(',')
                    intcd = int(trackdata[0])
                    inttrack = int(trackdata[1])
                    tmpcd = PC.DataProcessor.select_cd(lst_Inventory, intcd)
                    tmptrack = DC.Track(inttrack, trackdata[2], trackdata[3])
                    tmpcd.add_track(tmptrack)
