...
# EMalina, 2021-Dec-12, Added Select CD and Add Track functionality
#------------------------------------------#
...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        try:
            ...
            raise Exception('Selection not an integer')
        for row in table:
            if row.cd_id == cd_idx:
                return row
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        try:
            type(track_info[0]) is int
            track_pos, track_tit, track_lng = track_info
            int_track_pos = int(track_pos)
            tmp_track = DC.Track(int_track_pos, track_tit, track_lng)
